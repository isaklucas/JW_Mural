...
class FinalSemana:
    ...
    def selecionar_leitor_automaticamente(semanas):
        ...
        Em cada semana o leitor não pode ser o mesmo que o presidente; se o candidato
        na posição i for o presidente, avança na lista até achar outro.
        Altera semanas in-place; o modal usa a mesma referência e verá esses valores.
        """
        ...
        if not lista:
            for i in range(len(semanas)):
                semanas[i]['leitor_sentinela'] = ''
            return
        n = len(lista)
        for i in range(len(semanas)):
            presidente_esta_semana = (semanas[i].get('presidente') or '').strip()
            k0 = i % n
            leitor_escolhido = ''
            for j in range(n):
                candidato = lista[(k0 + j) % n]
                if (candidato or '').strip() != presidente_esta_semana:
                    leitor_escolhido = candidato
                    break
            semanas[i]['leitor_sentinela'] = leitor_escolhido
    ...
    def solicitar_dados_usuario(semanas, parent=None):
        ...
        for idx, semana in enumerate(semanas):
            rotulo = (semana.get('semana') or '').strip() or f"Semana {idx + 1}"
            frame = ttk.LabelFrame(
                scrollable,
                text=f"{rotulo}: {semana.get('titulo_estudo', '')[:50]}...",
                padding=15
            )
